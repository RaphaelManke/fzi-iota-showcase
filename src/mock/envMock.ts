...
export default class EnvironmentMock implements Environment {
    ...
    addVehicle(v: VehicleMock, x: number, y: number) {
        const worker = () => {
    ...
    addMarker(id: string, x: number, y: number) {
    ...
    private checkForMarkers(v: VehicleMock, x: number, y: number) {
        const f: Marker | undefined = this.markers.find((o: Marker) => 
        Math.sqrt(Math.pow(Math.abs(x - o.x), 2)
            + Math.pow(Math.abs(y - o.y), 2)) < EnvironmentMock.EPS)
        if (f) {
            v.markerDetected(f.id)
        }
    }

    updatePos(v: VehicleMock, x: number, y: number) {
        let m = this.vehicles.get(v)
        if (m == undefined) {
            throw new Error(v.getId() + " is not part of environment")
        }
        const position = {x: (m.pos.x + x) % EnvironmentMock.SIZE, y: (m.pos.y + y) % EnvironmentMock.SIZE}
        this.vehicles.set(v, {worker: m.worker, pos: position, interval: m.interval})
        this.events.emit('updatedPos', {id: v.getId(), x: position.x, y: position.y})

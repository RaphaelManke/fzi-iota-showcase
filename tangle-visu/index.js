...
const iota = require('@iota/core').composeAPI({provider: 'http://localhost:14265'});
...
io.on('connection', client => {
    ...
    sock.on('message', async function(topic) {
        ...
        const emit = async (tx, left = 50) => {
            const milestone = tx.address === coordinatorAddress || cooBundles.has(tx.bundle);
            if (milestone) {
            ...
            if (!txs.has(tx.hash)) {
                ...
                if (!txs.has(tx.branchTransaction) && left > 0) {
                    left--;
                    try {
                        const [branch] = await iota.getTransactionObjects([tx.branchTransaction])
                        if (branch) {
                ...
                if (!txs.has(tx.trunkTransaction) && left > 0) {
                    left--;
                    try {
                        const [trunk] = await iota.getTransactionObjects([tx.trunkTransaction])
                        if (trunk) {
                ...
                client.emit('tx', {trunk: tx.trunkTransaction, branch: tx.branchTransaction, hash: tx.hash, milestone});
            }
        }
        await emit(tx);
    })

...
export function getReservationSeed(seed: string, tripIndex: number) {
  return trytes(hashUntilCapacity(explode(getTripSeedTrits(seed, tripIndex)), checkedReservationCapacity));
}

export function getMetaInfoSeed(seed: string) {
  return trytes(hashUntilCapacity(explodeWithIndex(seed, 0), checkedMetaInfoCapacity));
}
...
function getTripSeedTrits(seed: string, index: number) {
  return hashUntilCapacity(explodeWithIndex(seed, index), TRIP_CHANNEL_CAPACITY);
}
